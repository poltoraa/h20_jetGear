...
$arrF=array("IBLOCK_ID"=>3,"PROPERTY_GOOD"=>$arResult["ID"]);
...
$arResult["RATE"]=ceil($arResult["RATE"]/$num);
$arResult["NUM"]=$num;
$arResult["BRAND"]=array();
if(!empty($arResult["PROPERTIES"]["BRAND"]["VALUE"]))
{
$arrFp=array("IBLOCK_ID"=>4,"NAME"=>$arResult["PROPERTIES"]["BRAND"]["VALUE"]);

$resp = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arrFp);
if($obEl = $resp->GetNextElement())	{
	$propsp = $obEl->GetProperties();
	$ar_resp = $obEl->GetFields();
	$arResult["BRAND"]["NAME"]=$ar_resp["NAME"];
	$arResult["BRAND"]["PICTURE"]=CFile::GetPath($ar_resp["PREVIEW_PICTURE"]);
	$arResult["BRAND"]["LINK"]=$ar_resp["DETAIL_PAGE_URL"];
...
$arResult["LIKE"]=unserialize($APPLICATION->get_cookie("LIKE"));


/*выбор цвета:*/

if (!empty($arResult["PROPERTIES"]["SOPUT_TOVAR"]["VALUE"]) && !empty($arResult["PROPERTIES"]["ATR_COLOR"]["VALUE"])){

	if (CModule::IncludeModule("iblock") && CModule::IncludeModule("main")){

		$arSelect = Array("ID", "IBLOCK_ID", "PROPERTY_ATR_COLOR", "DETAIL_PAGE_URL");
		$id = $arResult["PROPERTIES"]["SOPUT_TOVAR"]["VALUE"];
		$arFilter = Array("ID" =>  $id ,"IBLOCK_ID" => $arParams["IBLOCK_ID"], "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
		$res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);

		while($ob = $res->GetNextElement()){
			$arFields = $ob->GetFields();
			$arResult["SOPUT_DETAIL_PAGE_URL"][] = $arFields["DETAIL_PAGE_URL"];

			$arProps = $ob->GetProperties();
			$arResult["COLOR_SOPUT_TOVAR"][] = CFile::ResizeImageGet($arProps["ATR_COLOR"]["VALUE"], array('width'=>40, 'height'=>40), BX_RESIZE_IMAGE_PROPORTIONAL, true);
		}
	}
